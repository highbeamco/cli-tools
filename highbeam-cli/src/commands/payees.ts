...
import { toPayeeData } from "../features/dataschema/reps/PayeeRep";

export async function payeesAction(opts: { business?: string }) {
  const env = new Env();
  ...
  const { queryPayees } = await import("../features/payees/queries/queryPayees");
  ...
  const payees = await queryPayees(apis.v1, { businessGuid });
  const result = payees.map(toPayeeData);
