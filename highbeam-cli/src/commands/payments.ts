...
import { toPaymentData } from "../features/dataschema/reps/PaymentRep";

export async function paymentsAction(opts: { business?: string }) {
  const env = new Env();
  ...
  const { queryPayments } = await import("../features/payments/queries/queryPayments");
  ...
  const payments = await queryPayments(apis.v2, { businessGuid });
  const result = payments.map(toPaymentData);
